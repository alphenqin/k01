...
ATATEAM_EVIDENCE_PROMPT = (
    ...
    "最终仅输出一句话，不输出分析过程，格式为：存在{证据类型1}、{证据类型2}……，{依据各字段提炼出的关键恶意事实}。"
    "必须首先说明证据类型，证据类型可为：存在样本行为、存在开源报告提及、存在漏洞利用关联、"
    "存在恶意流量通信关联、存在其他恶意线索；多个证据类型使用顿号连接。"
    "随后根据各字段内容提炼关键细节，不要简单重复字段名称。"
    "sample_behavior需总结样本与IOC关系，如被恶意样本通信、下载、释放、作为C2或被多个恶意样本关联。"
    "source_links需识别并提炼报告来源、报告标题、恶意家族、APT组织、IOC角色或重要结论，并保留对应URL，格式为（参考报告：https://xxxxx）。"
    "如果source_links只有URL或无法提炼具体恶意事实，只输出存在开源报告提及，参考报告：https://xxxxx。"
    ...
    "不得编造不存在的信息；无法确认某项内容时只描述已有事实，"
    "不要输出报告内容未提供、无法提炼具体恶意事实、无法确认、未提供具体信息等消极说明。"
    "涉及中国攻击背景、中国关联APT、中国国家背景、疑似中国来源、中国攻击组织等地缘归因或攻击背景表述时，"
    "一律用“国家级背景攻击实体”代替，不要输出中国、中国关联、Chinese、China-nexus、China-linked、PRC、"
    "中国APT等原始归因措辞。"
